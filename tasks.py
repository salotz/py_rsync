...
def repo_test(cx):
    ...
    with cx.prefix(" py_rsync.dev"):
        cx.run("conda activate py_rsync.dev && inv -l",
               pty=True)
...
def docs_build(cx):
    ...
    with cx.cd('sphinx'):
        ...
        cx.run("sphinx-apidoc -f --separate --private --ext-autodoc --module-first --maxdepth 1 -o api ../src/py_rsync")
...
def lint(cx):
    ...
    cx.run("flake8 --output-file=metrics/lint/flake8.txt src/py_rsync")
...
def complexity(cx):
    ...
    cx.run("lizard -o metrics/code_quality/lizard.csv src/py_rsync")
    cx.run("lizard -o metrics/code_quality/lizard.html src/py_rsync")

    # SNIPPET: annoyingly opens the browser

    # make a cute word cloud of the things used
    # cx.run("(cd metrics/code_quality; lizard -EWordCount src/py_rsync > /dev/null)")
...
def version_which(cx):
    ...
    from py_rsync import __version__ as proj_version
    print(proj_version)
...
def publish_test_pypi(cx, version=None):
    ...
    # cx.run("twine upload "
    #        f"--repository-url {TESTING_INDEX_URL} "
    #        "dist/py_rsync-{version}*")


@task(pre=[clean_dist, update_tools, build_sdist])
def publish_test(cx):

    from py_rsync import __version__ as version
...
#     cx.run(f"cp conda/conda-forge {CONDA_FORGE_RECIPE_PATH}/py_rsync")
