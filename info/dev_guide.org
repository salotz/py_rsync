...
#+BEGIN_SRC bash
git clone https://github.com/salotz/py_rsync
cd py_rsync
#+END_SRC
...
To create an env called ~py_rsync.dev~ just run the ~env~ target from
~invoke~:
...
#+begin_src bash
pip install --index-url https://test.pypi.org/simple/ --no-deps py_rsync-salotz
#+end_src
